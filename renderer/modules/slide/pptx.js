...
export async function exportPptx(slides, theme, { fileName = '演示文稿' } = {}) {
  ...
  for (const s of slides) {
    ...
    for (const el of s.elements || []) {
      ...
      const rot = el.rotate ? { rotate: el.rotate } : {};
      if (el.type === 'text') {
        slide.addText(el.text || '', {
          x, y: yy, w, h, ...rot, fontSize: (el.fontSize || 1.4) * 14, color: (el.color || theme.fg).replace('#', ''),
        });
      } else if (el.type === 'rect') {
        slide.addShape('rect', { x, y: yy, w, h, ...rot, fill: { color: (el.fill || theme.accent).replace('#', '') } });
      } else if (el.type === 'ellipse') {
        slide.addShape('ellipse', { x, y: yy, w, h, ...rot, fill: { color: (el.fill || theme.accent).replace('#', '') } });
      } else if (el.type === 'image' && el.src) {
        try {
          if (el.src.startsWith('data:')) slide.addImage({ data: el.src, x, y: yy, w, h, ...rot });
          else slide.addImage({ path: el.src.replace(/^file:\/\//, ''), x, y: yy, w, h, ...rot });
