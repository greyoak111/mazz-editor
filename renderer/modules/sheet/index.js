  ...
  function renderTabs() {
    ...
    wb.sheets.forEach((s, i) => {
      ...
      el.addEventListener('dblclick', async () => {
        const name = await inputModal('重命名工作表', s.name);
        if (name?.trim() && wb.renameSheet(i, name.trim())) renderTabs();
      });
      el.addEventListener('contextmenu', async (e) => {
        e.preventDefault();
        if (wb.sheets.length > 1) {
          const r = await window.mazz.invoke('dialog:confirm', { title: '删除工作表', message: `删除工作表「${s.name}」？`, buttons: ['删除', '取消'] }).catch(() => null);
          if (r === 0 || r?.button === 0 || r === true) { wb.removeSheet(i); rebuildGrid(); renderTabs(); }
        }
      });
...
export default {
  ...
  toolbarHTML: `
    <div class="rb-group" data-label="字体">
      <div id="sg-font-picker"></div>
      <div id="sg-size-picker"></div>
    ...
    <div class="rb-group" data-label="颜色/边框">
  ...
  bindToolbar(panel) {
    ...
    new FontFamilyPicker(panel.querySelector('#sg-font-picker'), {
      onChange: (font) => font && window.MazzCommands.execute('sheet.setFont', { font }),
    });
    const sp = new FontSizePicker(panel.querySelector('#sg-size-picker'), {
      onChange: (size) => size && window.MazzCommands.execute('sheet.setFontSize', { size }),
    });
    sp.input.placeholder = '字号';
    new ColorPicker(panel.querySelector('#sg-color-picker'), {
      label: '文字色',
      onChange: (color) => window.MazzCommands.execute('sheet.setTextColor', { color }),
    });
    new ColorPicker(panel.querySelector('#sg-fill-picker'), {
      label: '填充色',
  ...
  contributes: {
    commands: [
      ...
      { id: 'sheet.setFont', title: '设置字体', group: '格式', when: "module=='sheet'", run: (p) => withSel((ctl, sel) => applyStyleToSel(ctl, sel, { font: p?.font }))() },
