...
export async function exportXlsx(workbook, { chartImages = [] } = {}) {
  ...
  for (const sheet of workbook.sheets) {
    ...
    for (const [k, cell] of sheet.cells) {
      ...
      const s = cell.s || {};
      if (s.bold || s.italic || s.underline || s.color) {
        ec.font = { bold: !!s.bold, italic: !!s.italic, underline: !!s.underline, color: s.color ? { argb: 'FF' + s.color.replace('#', '') } : undefined };
      }
      if (s.fill) ec.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + s.fill.replace('#', '') } };
      if (s.align) ec.alignment = { horizontal: s.align };
