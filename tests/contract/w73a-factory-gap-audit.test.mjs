...
describe('W73a Factory gap audit', () => {
  test('22 项恢复账只有五种明确分类且计数闭合', () => {
    ...
    assert.equal(matrix.coordinate, 'main@3b924c2');
    assert.equal(matrix.status, 'W73F_IMPLEMENTED');
  ...
  test('施工依赖从 W73a 到 W73h 单向，W73a-f 完成且唯一下一波是未批准 W73g', () => {
    ...
    assert.deepEqual(next.map(row => row.id), ['W73g']);
  ...
  test('规格冻结文件优先 Run、透明 Router、防 Goodhart 与审计停工线', () => {
    ...
    for (const needle of [
      'W73f IMPLEMENTED / W73g–h NOT APPROVED',
