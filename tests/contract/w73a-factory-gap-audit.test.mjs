...
describe('W73a Factory gap audit', () => {
  test('22 项恢复账只有五种明确分类且计数闭合', () => {
    ...
    assert.equal(matrix.coordinate, 'main@9ac443d');
    assert.equal(matrix.status, 'W73_COMPLETE');
  ...
  test('施工依赖从 W73a 到 W73h 单向且 W73 全波完成', () => {
    ...
    assert.equal(matrix.waves[7].status, 'COMPLETE_BY_THIS_CHANGE');
  ...
  test('规格冻结文件优先 Run、透明 Router、防 Goodhart 与审计停工线', () => {
    ...
    for (const needle of [
      ...
      '20 次生产/关闭',
      'main/factory-run-owners.js',
      'mazz.factory-runtime-convergence/v0',
