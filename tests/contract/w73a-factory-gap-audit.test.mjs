...
describe('W73a Factory gap audit', () => {
  test('22 项恢复账只有五种明确分类且计数闭合', () => {
    ...
    assert.equal(matrix.coordinate, 'main@d663cb1');
    assert.equal(matrix.status, 'W73E_IMPLEMENTED');
  ...
  test('施工依赖从 W73a 到 W73h 单向，W73a-e 完成且唯一下一波是未批准 W73f', () => {
    ...
    assert.deepEqual(next.map(row => row.id), ['W73f']);
  ...
  test('规格冻结文件优先 Run、透明 Router、防 Goodhart 与审计停工线', () => {
    ...
    for (const needle of [
      'W73e IMPLEMENTED / W73f–h NOT APPROVED',
