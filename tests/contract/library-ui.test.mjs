...
describe('书库 UI', () => {
  test('书架渲染 + 打开 epub 阅读 + 翻页 + 进度记忆', async () => {
    ...
    const epubCard = [...cards].find(card => card.dataset.id === 'bk1');
    assert.ok(epubCard?.textContent.includes('UI 测试书'), 'recent 排序不应改变按稳定 id 找书的语义');
    // 打开第一本
    ...
    // 进度已记忆
    const progress = await ctl.repository.getValue('progress');
  ...
  test('右键 10 号菜单：移出书架', async () => {
    ...
    libraryModule.create(container);
    await tick();
    const card = [...container.querySelectorAll('.lib-card')].find(node => node.dataset.id === 'bk1');
    assert.ok(card, '应按稳定 id 选中 EPUB，而不是依赖 recent 排序后的首卡');
    ...
    await tick(60);
    const ctl = instances.get(container);
    assert.equal((await ctl.repository.listBooks()).length, 1, '应移出一本');
