// tests/e2e/run36.mjs —— 波次四十七大修实证批 runner（scenes36 专用迭代器）
...
import { scenes36 } from './scenes36.mjs';
...
async function main() {
  ...
  await scenes36({ app, win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次四十七实证批：${results.length - fails.length}/${results.length} 过`);
