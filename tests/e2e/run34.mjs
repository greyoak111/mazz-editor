// tests/e2e/run34.mjs —— 波次四十三衍生面板并行化实证批 runner（scenes34 专用迭代器）
...
import { scenes34 } from './scenes34.mjs';
...
async function main() {
  ...
  await scenes34({ app, win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次四十三实证批：${results.length - fails.length}/${results.length} 过`);
