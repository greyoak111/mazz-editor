// tests/e2e/run76.mjs —— W61b 编辑回写与 1～4 路任务调度实证
...
import { scenes76 } from './scenes76.mjs';
...
const app = await electron.launch({
  args: [ROOT],
  env: { ...process.env, MAZZ_E2E_USER_DATA: USER_DATA, MAZZ_E2E_WORKSPACE: WS, MAZZ_E2E_FACTORY_MOCK: '1', MAZZ_E2E_FACTORY_DELAY_MS: '260', MAZZ_E2E_DISABLE_GPU: '1', NODE_ENV: 'test' },
  timeout: 120000,
});
...
const human = new Human(win, { tag: 'w61b' });
human.watchMain(app);
...
await scenes76({ app, win, human, WS, scenario, shotDir: SHOT_DIR, fs });
...
console.log(`W61b实证批：${passed}/${results.length} 过`);
