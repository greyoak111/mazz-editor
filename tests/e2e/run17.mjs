// tests/e2e/run17.mjs —— 波次二十八真机四瑕疵实证批 runner（scenes17 专用迭代器）
...
import { scenes17 } from './scenes17.mjs';
...
async function main() {
  ...
  await scenes17({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次二十八实证批：${results.length - fails.length}/${results.length} 过`);
