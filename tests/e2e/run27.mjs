// tests/e2e/run27.mjs —— 波次三十六mazzslide v2骨架实证批 runner（scenes27 专用迭代器）
...
import { scenes27 } from './scenes27.mjs';
...
async function main() {
  ...
  await scenes27({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次三十六实证批：${results.length - fails.length}/${results.length} 过`);
