// tests/e2e/run42.mjs —— 波次五十二之二工具坞三件套实证批 runner（scenes42 专用迭代器）
...
import { scenes42 } from './scenes42.mjs';
...
async function main() {
  ...
  await scenes42({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次五十二之二实证批：${results.length - fails.length}/${results.length} 过`);
