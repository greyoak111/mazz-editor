// tests/e2e/run38.mjs —— 波次四十九渲染方案调整实证批 runner（scenes38 专用迭代器）
...
import { scenes38 } from './scenes38.mjs';
...
async function main() {
  ...
  await scenes38({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次四十九实证批：${results.length - fails.length}/${results.length} 过`);
