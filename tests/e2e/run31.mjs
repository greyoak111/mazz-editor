// tests/e2e/run31.mjs —— 波次四十手机遥控实证批 runner（scenes31 专用迭代器）
...
import { scenes31 } from './scenes31.mjs';
...
async function main() {
  ...
  await scenes31({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次四十实证批：${results.length - fails.length}/${results.length} 过`);
