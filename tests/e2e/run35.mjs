// tests/e2e/run35.mjs —— 波次四十四播放器媒体库专修实证批 runner（scenes35 专用迭代器）
...
import { scenes35 } from './scenes35.mjs';
...
async function main() {
  ...
  await scenes35({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次四十四实证批：${results.length - fails.length}/${results.length} 过`);
