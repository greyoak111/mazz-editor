// tests/e2e/run45.mjs —— W52e收官实证批 runner（scenes45 专用迭代器）
...
import { scenes45 } from './scenes45.mjs';
...
async function main() {
  ...
  await scenes45({ app, win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`W52e实证批：${results.length - fails.length}/${results.length} 过`);
