// tests/e2e/run20.mjs —— 波次三十一图形库扩容模板包模块骨架实证批 runner（scenes20 专用迭代器）
...
import { scenes20 } from './scenes20.mjs';
...
async function main() {
  ...
  await scenes20({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次三十一实证批：${results.length - fails.length}/${results.length} 过`);
