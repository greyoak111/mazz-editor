// tests/e2e/run21.mjs —— 波次三十二性能碾压实证批 runner（scenes21 专用迭代器）
...
import { scenes21 } from './scenes21.mjs';
...
async function main() {
  ...
  await scenes21({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次三十二实证批：${results.length - fails.length}/${results.length} 过`);
