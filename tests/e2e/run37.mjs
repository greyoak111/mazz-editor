// tests/e2e/run37.mjs —— 波次四十八真机三小改实证批 runner（scenes37 专用迭代器）
...
import { scenes37 } from './scenes37.mjs';
...
async function main() {
  ...
  await scenes37({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次四十八实证批：${results.length - fails.length}/${results.length} 过`);
