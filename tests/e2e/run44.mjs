// tests/e2e/run44.mjs —— 波次五十二之四收官实证批 runner（scenes44 专用迭代器）
...
import { scenes44 } from './scenes44.mjs';
...
async function main() {
  ...
  await scenes44({ app, win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次五十二之四实证批：${results.length - fails.length}/${results.length} 过`);
