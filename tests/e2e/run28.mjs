// tests/e2e/run28.mjs —— 波次三十七画布编辑层实证批 runner（scenes28 专用迭代器）
...
import { scenes28 } from './scenes28.mjs';
...
async function main() {
  ...
  await scenes28({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次三十七实证批：${results.length - fails.length}/${results.length} 过`);
