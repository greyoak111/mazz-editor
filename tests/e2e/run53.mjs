// tests/e2e/run53.mjs —— W53救火收官实证批 runner（scenes53 专用迭代器）
...
import { scenes53 } from './scenes53.mjs';
...
async function main() {
  ...
  await scenes53({ app, win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`W53救火实证批：${results.length - fails.length}/${results.length} 过`);
