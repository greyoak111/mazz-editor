// tests/e2e/run30.mjs —— 波次三十九放映引擎实证批 runner（scenes30 专用迭代器）
...
import { scenes30 } from './scenes30.mjs';
...
async function main() {
  ...
  await scenes30({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次三十九实证批：${results.length - fails.length}/${results.length} 过`);
