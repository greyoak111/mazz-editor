// tests/e2e/run14.mjs —— 波次二十六播放器列表栏工作区栏同款实证批 runner（scenes14 专用迭代器）
...
import { scenes14 } from './scenes14.mjs';
...
async function main() {
  ...
  await scenes14({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次二十六实证批：${results.length - fails.length}/${results.length} 过`);
