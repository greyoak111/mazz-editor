// tests/e2e/run24.mjs —— 波次三十四窗格三模式批量选删导图桥接实证批 runner（scenes24 专用迭代器）
...
import { scenes24 } from './scenes24.mjs';
...
async function main() {
  ...
  await scenes24({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次三十四实证批：${results.length - fails.length}/${results.length} 过`);
