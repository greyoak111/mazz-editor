// tests/e2e/run40.mjs —— 波次五十一体验根治实证批 runner（scenes40 专用迭代器）
...
import { scenes40 } from './scenes40.mjs';
...
async function main() {
  ...
  await scenes40({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次五十一实证批：${results.length - fails.length}/${results.length} 过`);
