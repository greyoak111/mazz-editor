// tests/e2e/run32.mjs —— 波次四十一键成页实证批 runner（scenes32 专用迭代器）
...
import { scenes32 } from './scenes32.mjs';
...
async function main() {
  ...
  await scenes32({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次四十一实证批：${results.length - fails.length}/${results.length} 过`);
