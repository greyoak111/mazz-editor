// tests/e2e/run13.mjs —— 波次二十五DMHY首页全编码音轨种子内字幕实证批 runner（scenes13 专用迭代器）
...
import { scenes13 } from './scenes13.mjs';
...
async function main() {
  ...
  await scenes13({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次二十五实证批：${results.length - fails.length}/${results.length} 过`);
