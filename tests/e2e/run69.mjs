// tests/e2e/scenes69.mjs —— 59d 实证批 runner（scenes69 专用迭代器）
...
import { scenes69 } from './scenes69.mjs';
...
fs.writeFileSync(WS + '/测试文档.md', '# 59d 实证\n\n正文。\n');
...
async function main() {
  ...
  await scenes69({ app, win, human, WS, WS2, scenario });
  ...
  console.log('═══════════');
  console.log(`59d实证批：${passed}/${results.length} 过`);
