// tests/e2e/run71.mjs —— W60a 本地 mock runner
...
import { scenes71 } from './scenes71.mjs';
...
const app = await electron.launch({
  args: [ROOT],
  env: { ...process.env, MAZZ_E2E_USER_DATA: USER_DATA, MAZZ_E2E_WORKSPACE: WS, MAZZ_E2E_FACTORY_MOCK: '1', NODE_ENV: 'test' },
  timeout: 120000,
});
const win = await app.firstWindow();
// firstWindow 可能在 DOMContentLoaded 已经越过后才返回；以可重复读取的 DOM 状态为准。
...
await scenes71({ win, human, WS, scenario, shotDir: SHOT_DIR });
...
console.log(`W60a实证批：${passed}/${results.length} 过`);
