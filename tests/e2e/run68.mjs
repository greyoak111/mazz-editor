// tests/e2e/scenes68.mjs —— 59c 实证批 runner（scenes68 专用迭代器）
...
import { scenes68 } from './scenes68.mjs';
...
fs.writeFileSync(WS + '/测试文档.md', '# 59c 实证\n\n正文。\n');
...
async function main() {
  ...
  await scenes68({ app, win, human, WS, WS2, scenario });
  ...
  console.log('═══════════');
  console.log(`59c实证批：${passed}/${results.length} 过`);
