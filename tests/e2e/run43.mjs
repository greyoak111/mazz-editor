// tests/e2e/run43.mjs —— 波次五十二之三浮层遣散实证批 runner（scenes43 专用迭代器）
...
import { scenes43 } from './scenes43.mjs';
...
async function main() {
  ...
  await scenes43({ app, win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次五十二之三实证批：${results.length - fails.length}/${results.length} 过`);
