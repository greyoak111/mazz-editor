// tests/e2e/run18.mjs —— 波次二十九浏览器白屏mkv容错实证批 runner（scenes18 专用迭代器）
...
import { scenes18 } from './scenes18.mjs';
...
async function main() {
  ...
  await scenes18({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次二十九实证批：${results.length - fails.length}/${results.length} 过`);
