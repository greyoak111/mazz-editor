// tests/e2e/run41.mjs —— 波次五十二地基回正实证批 runner（scenes41 专用迭代器）
...
import { scenes41 } from './scenes41.mjs';
...
async function main() {
  ...
  await scenes41({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次五十二实证批：${results.length - fails.length}/${results.length} 过`);
