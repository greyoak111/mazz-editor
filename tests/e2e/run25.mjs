// tests/e2e/run25.mjs —— 混合画布快捷键对调实证批 runner（scenes25 专用迭代器）
...
import { scenes25 } from './scenes25.mjs';
...
async function main() {
  ...
  await scenes25({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`混合画布实证批：${results.length - fails.length}/${results.length} 过`);
