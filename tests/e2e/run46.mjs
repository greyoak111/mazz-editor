// tests/e2e/run46.mjs —— W52e②收官实证批 runner（scenes46 专用迭代器）
...
import { scenes46 } from './scenes46.mjs';
...
async function main() {
  ...
  await scenes46({ app, win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`W52e②实证批：${results.length - fails.length}/${results.length} 过`);
