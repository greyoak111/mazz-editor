// tests/e2e/run29.mjs —— 波次三十八编排层实证批 runner（scenes29 专用迭代器）
...
import { scenes29 } from './scenes29.mjs';
...
async function main() {
  ...
  await scenes29({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次三十八实证批：${results.length - fails.length}/${results.length} 过`);
