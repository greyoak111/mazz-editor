// tests/e2e/run33.mjs —— 波次四十二pptx互通实证批 runner（scenes33 专用迭代器）
...
import { scenes33 } from './scenes33.mjs';
...
async function main() {
  ...
  await scenes33({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次四十二实证批：${results.length - fails.length}/${results.length} 过`);
