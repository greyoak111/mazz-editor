// tests/e2e/run39.mjs —— 波次五十渲染根治实证批 runner（scenes39 专用迭代器）
...
import { scenes39 } from './scenes39.mjs';
...
async function main() {
  ...
  await scenes39({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次五十实证批：${results.length - fails.length}/${results.length} 过`);
