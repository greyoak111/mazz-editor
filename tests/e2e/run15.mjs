// tests/e2e/run15.mjs —— 波次二十七页面同源化解码正名实证批 runner（scenes15 专用迭代器）
...
import { scenes15 } from './scenes15.mjs';
...
async function main() {
  ...
  await scenes15({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次二十七实证批：${results.length - fails.length}/${results.length} 过`);
