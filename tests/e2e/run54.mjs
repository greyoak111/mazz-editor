// tests/e2e/run54.mjs —— W56救火②收官实证批 runner（scenes54 专用迭代器）
...
import { scenes54 } from './scenes54.mjs';
...
async function main() {
  ...
  await scenes54({ app, win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`W56救火②实证批：${results.length - fails.length}/${results.length} 过`);
