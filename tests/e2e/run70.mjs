// tests/e2e/scenes70.mjs —— 59e 实证批 runner（scenes70 专用迭代器）
...
import { scenes70 } from './scenes70.mjs';
...
fs.writeFileSync(WS + '/测试文档.md', '# 59e 实证\n\n正文。\n');
...
async function main() {
  ...
  await scenes70({ app, win, human, WS, WS2, scenario });
  ...
  console.log('═══════════');
  console.log(`59e实证批：${passed}/${results.length} 过`);
