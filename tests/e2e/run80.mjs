// W68b Factory Desk 活稿车间专项 Electron 实证。
...
import { scenes80 } from './scenes80.mjs';
...
const human = new Human(win, { tag: 'w68b' });
human.watchMain(app);
...
await scenes80({ app, win, human, scenario, shotDir: SHOT_DIR });
await scenario('异常警察·W68b 全程零主进程/渲染异常', async () => { await human.finish({ allow: ['ERR_ABORTED', 'net::ERR_FILE_NOT_FOUND', 'favicon'] }); });
const passed = results.filter(row => row[1] === 'PASS').length;
console.log(`W68b 实证批：${passed}/${results.length} 通过`);
