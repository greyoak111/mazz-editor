// tests/e2e/run22.mjs —— 波次三十三演示叙事模式实证批 runner（scenes22 专用迭代器）
...
import { scenes22 } from './scenes22.mjs';
...
async function main() {
  ...
  await scenes22({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次三十三实证批：${results.length - fails.length}/${results.length} 过`);
