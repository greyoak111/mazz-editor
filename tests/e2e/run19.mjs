// tests/e2e/run19.mjs —— 波次三十导图渲染病根治实证批 runner（scenes19 专用迭代器）
...
import { scenes19 } from './scenes19.mjs';
...
async function main() {
  ...
  await scenes19({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次三十实证批：${results.length - fails.length}/${results.length} 过`);
