// tests/e2e/run23.mjs —— 白屏根治原生右键菜单invalidate实证批 runner（scenes23 专用迭代器）
...
import { scenes23 } from './scenes23.mjs';
...
async function main() {
  ...
  await scenes23({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`白屏根治实证批：${results.length - fails.length}/${results.length} 过`);
