// tests/e2e/run16.mjs —— 波次二十七c HEVC官方组件指引实证批 runner（scenes16 专用迭代器）
...
import { scenes16 } from './scenes16.mjs';
...
async function main() {
  ...
  await scenes16({ win, human, WS, WS2, scenario });
  ...
  console.log('\n═══════════');
  console.log(`波次二十七c实证批：${results.length - fails.length}/${results.length} 过`);
