// tests/e2e/run74.mjs —— W60b 立项向导与 Output 协议实证
...
import { scenes74 } from './scenes74.mjs';
...
const human = new Human(win, { tag: 'w60b' });
human.watchMain(app);
...
await win.waitForTimeout(900); // 协议首启为动态 import，先等它出现再清场
for (let i = 0; i < 20; i++) {
...
await scenes74({ app, win, human, WS, scenario, shotDir: SHOT_DIR });
...
console.log(`W60b实证批：${passed}/${results.length} 过`);
