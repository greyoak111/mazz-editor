...
const workaroundDefinitions = [
  ...
  { id: 'WKR-DRAG-CLOAK', owner: 'Shell/BrowserViews/VisualComposition', status: 'KEEP', pattern: /captureVisibleHost|dragCloak\s*\(/, reason: 'Split drag captures, decodes and paints every visible WCV for the sender host, validates the exact tabId/webContentsId set, then applies temporary host-wide cloak; capture failure stays visible and proxy removal waits for native visibility plus fresh pixels.' },
  { id: 'WKR-PANE-MOVE-RESYNC', owner: 'Shell/Browser module', status: 'KEEP', pattern: /pane:tabMoved/, reason: 'Native view host move requires post-layout bounds/recompose/invalidate convergence without network reload.' },
...
const surfaceCensus = {
  ...
  protocolReality: {
    ...
    host: 'All managed windows, panel windows, WebContentsViews and registered DOM overlays carry an explicit host; overlay tokens occlude native surfaces per host with reference counting, and split-drag may occlude only after exact per-host tabId/webContentsId coverage validation.',
  ...
  surfaceV1InterfaceDraft: {
    ...
    invariants: [
      ...
      'Split-drag may occlude a host only after every captured WCV proxy is decoded, painted and identity-validated; proxy removal waits for native visibility and fresh pixels.',
